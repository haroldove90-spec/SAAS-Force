  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full">
      <div className="p-4 border-b border-slate-100 bg-slate-50/50">
        <h2 className="font-bold text-slate-700 text-sm">Resumen de Pagos</h2>
      </div>
          ...
          <div key={payment.id} className="flex justify-between items-center pb-3 border-b border-slate-50 last:border-0 last:pb-0">
            <div>
              ...
              <div className="text-[10px] text-slate-400 uppercase font-medium">
                {payment.metodo.replace('_', ' ')} • {payment.pago_referencia || 'SIN REF'}
              </div>
            </div>
            <span className={cn(
      ...
      <button 
        onClick={() => {}} 
        className="w-full py-3 bg-slate-50 text-slate-500 font-bold text-[10px] uppercase tracking-wider hover:bg-slate-100 transition-colors border-t border-slate-100"
      >
